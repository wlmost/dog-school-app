...
This specification defines how the system captures and stores the
history a dog has with its current owner: since when the dog has been
with the owner, roughly how old the dog was when it arrived, and where
the dog came from. This information helps trainers assess prior
experience, especially for dogs from animal shelters. It covers both the
admin/trainer dog record and the customer self-service dog registration
flow, including how the fields carry over when a registration request is
approved.
