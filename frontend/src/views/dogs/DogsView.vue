      ...
      <div v-for="dog in dogs" :key="dog.id" class="card hover:shadow-lg transition-shadow cursor-pointer" @click="viewDog(dog)">
        ...
        <div class="space-y-2 mb-4">
          <div class="flex items-center text-sm text-gray-600">
            ...
            {{ dog.customer?.user?.full_name || '-' }}
          </div>
          <div v-if="dog.date_of_birth" class="flex items-center text-sm text-gray-600">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            {{ formatDate(dog.date_of_birth) }}
          </div>
          <div v-if="dog.chip_number" class="flex items-center text-sm text-gray-600">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
